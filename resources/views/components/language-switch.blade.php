...
<div x-data="{ open: false }" class="relative">
    ...
    <div x-show="open" @click.away="open = false"
        class="absolute right-0 mt-2 w-40 bg-white border border-teal-200 rounded-lg shadow-lg z-50">
        @foreach ($languages as $lang)
            <a href="{{ switch_locale_url($lang->code) }}"
                class="block px-4 py-2 text-sm text-gray-700 hover:bg-teal-100">
                {{ $lang->name }}
            </a>
        @endforeach
    </div>
