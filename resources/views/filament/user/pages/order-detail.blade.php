            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <div class="text-gray-600 space-y-4">
                        ...
                        <p class="flex justify-between">
                            ...
                            <span class="text-teal-600">{{ $order->status->label() }}</span>
                        </p>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="text-gray-600 space-y-4">
                        ...
                        <p>{{ $order->shippingAddress->zone?->zoneTranslations->where('language_id', $lang->id)->first()?->name ?? $order->shippingAddress->zone?->name ?? '' }}, {{ $order->shippingAddress->country->countryTranslations->where('language_id', $lang->id)->first()?->name ?? $order->shippingAddress->country->name ?? '' }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="border-b border-gray-200">
                        <tr>
                            <th class="text-left py-3 text-sm font-medium text-gray-700">{{ __('orders.product_name') }}</th>
                            <th class="text-center py-3 text-sm font-medium text-gray-700">{{ __('orders.quantity') }}</th>
                            <th class="text-right py-3 text-sm font-medium text-gray-700">{{ __('orders.unit_price') }}</th>
                            <th class="text-right py-3 text-sm font-medium text-gray-700">{{ __('orders.subtotal') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($order->orderItems as $item)
                            ...
                            <tr>
                                ...
                                <td class="text-right py-4">{{ $localeService->formatWithSymbol($item->price, $orderCurrency->code) }}</td>
                                <td class="text-right py-4">{{ $localeService->formatWithSymbol($item->price * $item->qty, $orderCurrency->code) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot class="border-t border-gray-200">
                        <tr>
                            ...
                            <td class="text-right py-4">{{ $localeService->formatWithSymbol($order->shipping_fee, $orderCurrency->code) }}</td>
                        </tr>
                        <tr>
                            <td colspan="3" class="text-right py-4 font-semibold">{{ __('orders.total') }}:</td>
                            <td class="text-right py-4 font-semibold text-teal-600">{{ $localeService->formatWithSymbol($order->total, $orderCurrency->code) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
