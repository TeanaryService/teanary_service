@php
    $breadcrumbs = buildManagerCenterBreadcrumbs('articles', __('filament.ArticleResource.label'));
@endphp
        ...
        <div class="flex flex-col md:flex-row gap-6">
            ...
            <div class="flex-1">
                <div class="mb-6 flex items-center justify-between">
                    <h1 class="text-3xl font-bold text-gray-900">{{ __('filament.ArticleResource.label') }}</h1>
                    <a href="{{ locaRoute('manager.articles.create') }}" 
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select 
                                wire:model.live="filterIsPublished" 
                                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500"
                            >
                                <option value="">{{ __('app.all') }}</option>
                                <option value="1">{{ __('filament.article.published') }}</option>
                                <option value="0">{{ __('filament.article.unpublished') }}</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                {{ __('filament.article.translation_status') }}
                            </label>
                            <select 
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            ...
                                            @else
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                                    ...
                                                    {{ __('filament.article.unpublished') }}
                                                </span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-2">
                                                <button 
                                                    ...
                                                    title="{{ $article->is_published ? __('filament.article.unpublish') : __('filament.article.publish') }}"
                                                >
                                                    @if($article->is_published)
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
