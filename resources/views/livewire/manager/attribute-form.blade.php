@php
    ...
    $breadcrumbs = buildManagerCenterBreadcrumbs('attributes', $isEdit ? __('app.edit') : __('app.create'), __('filament.AttributeResource.label'), locaRoute('manager.attributes'));
@endphp
            ...
            <div class="flex-1">
                <div class="mb-6">
                    <h1 class="text-3xl font-bold text-gray-900">
                        {{ $isEdit ? __('app.edit') : __('app.create') }} {{ __('filament.AttributeResource.label') }}
                    </h1>
                </div>
                ...
                <form wire:submit="save" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="space-y-6">
                        {{-- 基本信息 --}}
                        <div>
                            <h2 class="text-lg font-semibold text-gray-900 mb-4">{{ __('filament.attribute.basic_info') }}</h2>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">{{ __('filament.attribute.is_filterable_helper') }}</p>
                                </div>

                                {{-- 翻译状态 --}}
                                <div>
                                    <label for="translationStatus" class="block text-sm font-medium text-gray-700 mb-2">
                                        {{ __('filament.attribute.translation_status') }} <span class="text-red-500">*</span>
                                    </label>
                                    <select 
                        ...
                        {{-- 翻译 --}}
                        <div>
                            <h2 class="text-lg font-semibold text-gray-900 mb-4">{{ __('filament.attribute.translations') }}</h2>
                            <div class="space-y-4">
                                @foreach($languages as $language)
                                    <div>
                                        <label for="name_{{ $language->id }}" class="block text-sm font-medium text-gray-700 mb-2">
                                            {{ __('filament.attribute.name') }} ({{ $language->name }})
                                            @if($language->default)
                                        ...
                                        @enderror
                                        @if($language->default)
                                            <p class="mt-1 text-xs text-gray-500">{{ __('filament.attribute.name_helper') }}</p>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
