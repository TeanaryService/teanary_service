            ...
            <div class="flex-1">
                ...
                @if($this->hasSelectedItems())
                    <x-manager.batch-actions 
                        deleteMethod="batchDeleteContacts"
                    />
                @endif
                                ...
                                @forelse($contacts as $contact)
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <x-widgets.checkbox 
                                                standalone
                                                wireClick="toggleSelect({{ $contact->id }})"
                                                :checked="in_array($contact->id, $selectedItems)"
                                            />
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                            <a href="mailto:{{ $contact->email }}" class="text-teal-600 hover:text-teal-700">
                                                {{ $contact->email }}
                                            </a>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-600">
                                            <div class="max-w-md">{{ \Illuminate\Support\Str::limit($contact->message, 100) }}</div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-2">
                                                <button 
                                                    onclick="alert('{{ addslashes($contact->message) }}')"
...
<x-seo-meta title="{{ __('manager.contacts.label') }}" />
