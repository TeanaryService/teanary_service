        ...
        <div class="flex flex-col md:flex-row gap-6">
            ...
            <div class="flex-1">
                <x-widgets.page-header 
                    :title="__('manager.currencies.label')"
                >
                    <x-slot:actions>
                        <x-widgets.button href="{{ locaRoute('manager.currencies.create') }}" wire:navigate class="inline-flex items-center gap-2">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            <x-widgets.label>{{ __('manager.currency.exchange_rate_from') }}</x-widgets.label>
                            <x-widgets.input 
                                type="number" 
                                step="0.0001"
                                wire="live.debounce.300ms=exchangeRateFrom"
                                placeholder="0.0000"
                            />
                        </div>
                        <div>
                            <x-widgets.label>{{ __('manager.currency.exchange_rate_until') }}</x-widgets.label>
                            <x-widgets.input 
                                type="number" 
                                step="0.0001"
                                wire="live.debounce.300ms=exchangeRateUntil"
                                placeholder="0.0000"
                            />
                        </div>
                    </div>
                ...
                @if($this->hasSelectedItems())
                    <x-manager.batch-actions 
                        deleteMethod="batchDeleteCurrencies"
                    />
                @endif
                                ...
                                @forelse($currencies as $currency)
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <x-widgets.checkbox 
                                                standalone
                                                wireClick="toggleSelect({{ $currency->id }})"
                                                :checked="in_array($currency->id, $selectedItems)"
                                            />
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ locaRoute('manager.currencies.edit', ['id' => $currency->id]) }}" wire:navigate 
