@php
    $isEdit = $currencyId !== null;
    $breadcrumbs = buildManagerCenterBreadcrumbs('currencies', $isEdit ? __('app.edit') : __('app.create'), __('manager.currencies.label'), locaRoute('manager.currencies'));
@endphp

<div class="min-h-[70vh] mb-10 bg-tea-50 tea-bg-texture">
    <div class="w-full max-w-screen 2xl:max-w-[80vw] mx-auto px-6 md:px-8">
        <x-widgets.breadcrumbs :items="$breadcrumbs" />
        
        <div class="flex flex-col md:flex-row gap-6">
            <x-manager.sidebar active="currencies" />
            
            <div class="flex-1">
                <div class="mb-6">
                    <h1 class="text-3xl font-bold text-gray-900">
                        {{ $isEdit ? __('app.edit') : __('app.create') }} {{ __('manager.currencies.label') }}
                    </h1>
                </div>

                @if (session()->has('message'))
                    <div class="mb-4 rounded-md bg-teal-100 p-4">
                        <p class="text-sm font-medium text-teal-800">{{ session('message') }}</p>
                    </div>
                @endif

                <form wire:submit="save" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="space-y-6">
                        {{-- 基本信息 --}}
                        <div>
                            <h2 class="text-lg font-semibold text-gray-900 mb-4">{{ __('manager.currency.basic_info') }}</h2>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {{-- 货币代码 --}}
                                <x-widgets.form-field 
                                    :label="__('manager.currency.code')"
                                    labelFor="code"
                                    required
                                    error="code"
                                    :help="__('manager.currency.code_helper')"
                                >
                                    <x-widgets.input 
                                        type="text" 
                                        id="code"
                                        wire="code"
                                        placeholder="例如: USD"
                                        error="code"
                                    />
                                </x-widgets.form-field>

                                {{-- 货币名称 --}}
                                <x-widgets.form-field 
                                    :label="__('manager.currency.name')"
                                    labelFor="name"
                                    required
                                    error="name"
                                >
                                    <x-widgets.input 
                                        type="text" 
                                        id="name"
                                        wire="name"
                                        placeholder="例如: 美元"
                                        error="name"
                                    />
                                </x-widgets.form-field>

                                {{-- 货币符号 --}}
                                <x-widgets.form-field 
                                    :label="__('manager.currency.symbol')"
                                    labelFor="symbol"
                                    required
                                    error="symbol"
                                    :help="__('manager.currency.symbol_helper')"
                                >
                                    <x-widgets.input 
                                        type="text" 
                                        id="symbol"
                                        wire="symbol"
                                        placeholder="例如: $"
                                        error="symbol"
                                    />
                                </x-widgets.form-field>

                                {{-- 汇率 --}}
                                <x-widgets.form-field 
                                    :label="__('manager.currency.exchange_rate')"
                                    labelFor="exchangeRate"
                                    required
                                    error="exchangeRate"
                                    :help="__('manager.currency.exchange_rate_helper')"
                                >
                                    <x-widgets.input 
                                        type="number" 
                                        id="exchangeRate"
                                        step="0.0001"
                                        wire="exchangeRate"
                                        placeholder="1.0000"
                                        error="exchangeRate"
                                    />
                                </x-widgets.form-field>

                                {{-- 是否默认 --}}
                                <div class="md:col-span-2">
                                    <x-widgets.checkbox 
                                        wire="live=default"
                                        :label="__('manager.language.is_default')"
                                    />
                                    <p class="mt-1 text-xs text-gray-500">{{ __('manager.language.is_default_helper') }}</p>
                                </div>
                            </div>
                        </div>

                        {{-- 操作按钮 --}}
                        <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-200">
                            <x-widgets.button 
                                href="{{ locaRoute('manager.currencies') }}" wire:navigate 
                                variant="secondary"
                            >
                                {{ __('app.cancel') }}
                            </x-widgets.button>
                            <x-widgets.button type="submit">
                                {{ __('app.save') }}
                            </x-widgets.button>
                        </div>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>

<x-seo-meta title="{{ $isEdit ? __('app.edit') : __('app.create') }} {{ __('manager.currencies.label') }}" keywords="{{ __('manager.currencies.label') }}" />
