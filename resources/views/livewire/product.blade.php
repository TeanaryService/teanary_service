    ...
    <div class="w-full max-w-screen 2xl:max-w-[75vw] mx-auto px-6 md:px-8">
        ...
        <div class="flex flex-col md:flex-row gap-8">
            ...
            <aside class="md:w-1/4 tea-card rounded-xl p-6">
                <h2 class="text-xl font-bold text-teal-800 mb-4 tea-title">{{ __('home.browse_categories') }}</h2>
                <ul class="space-y-2">
                    @foreach ($categories as $category)
                        <li>
                            <a href="{{ locaRoute('product', ['slug' => $category['slug']]) }}" wire:navigate
                                class="flex gap gap-2 items-center px-4 py-2 rounded hover:bg-teal-100 transition-colors {{ $categoryId == $category['id'] ? 'bg-teal-200 font-bold text-teal-800' : 'text-teal-700' }}">
                                    ...
                                    @foreach ($category['children'] as $child)
                                        <li>
                                            <a href="{{ locaRoute('product', ['slug' => $child['slug']]) }}" wire:navigate
                                                class="flex gap gap-2 items-center px-3 py-1 rounded hover: transition-colors {{ $categoryId == $child['id'] ? 'bg-teal-200 font-bold text-teal-800' : 'text-teal-600' }}">
                ...
                <div class="mt-8 pt-8 border-t border-teal-500">
                    <h2 class="text-xl font-bold text-teal-800 mb-4 tea-title">{{ __('home.attributes') }}</h2>
                    <form method="GET" action="{{ locaRoute('product') }}">
                        @foreach ($filterAttributes as $index => $attr)
                            ...
                            <div class="mb-5 {{ $index > 0 ? 'pt-5 border-t border-teal-500' : '' }}">
                                <div class="font-semibold text-teal-700 mb-3 text-sm uppercase tracking-wide">{{ $attr['name'] }}</div>
                                <div class="space-y-2">
                                    @foreach ($attr['values'] as $val)
                                        ...
                                        @endphp
                                        <label class="flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer transition-all duration-200 {{ $isChecked ? 'bg-teal-100 text-teal-800 font-medium' : 'hover: text-teal-600' }}">
                                            <input 
                                                type="checkbox" 
                                                name="attributes[{{ $attr['id'] }}][]"
                                                value="{{ $val['id'] }}"
                                                {{ $isChecked ? 'checked' : '' }}
                                                class="w-4 h-4 text-teal-600 border-teal-300 rounded focus:ring-teal-500 focus:ring-2 cursor-pointer"
                                            />
                        ...
                        <div class="mt-6 flex gap-2">
                            <x-widgets.button 
                                type="submit"
                                class="flex-1 bg-teal-600 hover:bg-teal-700 text-white"
                            >
                                {{ __('home.search') }}
                            </x-widgets.button>
                            ...
                            <a 
                                ...
                                class="px-4 py-2 rounded-lg border border-teal-300 text-teal-700 hover: transition-colors duration-200 text-sm font-medium"
                            >
                                {{ __('app.reset') }}
                            </a>
                            @endif
